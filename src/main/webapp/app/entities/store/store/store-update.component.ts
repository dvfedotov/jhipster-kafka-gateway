...
export class StoreUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ store }) => {
      if (!store.id) {
        ...
        store.createTimestamp = today;
        store.updateTimestamp = today;
      }
  ...
  updateForm(store: IStore): void {
    this.editForm.patchValue({
      ...
      status: store.status,
      createTimestamp: store.createTimestamp ? store.createTimestamp.format(DATE_TIME_FORMAT) : null,
      updateTimestamp: store.updateTimestamp ? store.updateTimestamp.format(DATE_TIME_FORMAT) : null,
    });
  }
  ...
  private createFromForm(): IStore {
    return {
      ...
      createTimestamp: this.editForm.get(['createTimestamp'])!.value
        ...
        : undefined,
      updateTimestamp: this.editForm.get(['updateTimestamp'])!.value
